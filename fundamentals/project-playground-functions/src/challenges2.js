...
function auxTriangleCheck(lineA, lineB, lineC) {
  const conditionOne = lineA < lineB + lineC && lineA > Math.abs(lineB - lineC);
  const conditionTwo = lineB < lineA + lineC && lineB > Math.abs(lineA - lineC);
  const conditonThree = lineC < lineA + lineB && lineC > Math.abs(lineA - lineB);
...
function triangleCheck(lineA, lineB, lineC) {
  const { conditionOne, conditionTwo, conditonThree } = auxTriangleCheck(lineA, lineB, lineC);

  if (conditionOne && conditionTwo && conditonThree) return true;
  return false;
}
// Desafio 12 - Teste
